...
def log():
    ...
    result = db.collection7.find_one(query) #retrieve stored feature set from database
...
def reg():
    ...
    try:
        insert_result = db.collection7.insert_one({"length": length, "description": description, "user_id": user_id})
